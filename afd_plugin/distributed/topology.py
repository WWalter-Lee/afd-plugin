...
class AFDWindowExpertLayout:
    ...
    Rank 0 owns the shared expert.  Remaining FFN ranks own contiguous routed
    expert ranges.  Each routed rank allocates only its actual experts; the
    local FusedMoE is deliberately independent of the surrounding FFN DP
    group, so ranks do not need identical local expert counts.
    """
...
def build_window_expert_layout(
    *,
    ...
    """Return the deterministic shared-first expert layout for one FFN rank."""
