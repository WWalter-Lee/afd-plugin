...
def _fail_if_unsupported_deepseek_v4_features(
    ...
    if speculative_config is not None:
        ...
        if afd_config.num_attention_ranks != afd_config.num_ffn_ranks:
            raise RuntimeError(
                f"DeepSeek-V4 AFD {speculative_feature} requires equal A/F ranks"
            )
        if bool(getattr(parallel_config, "use_ubatching", False)):
            raise RuntimeError(
                f"DeepSeek-V4 AFD {speculative_feature} supports only U1"
            )
        ...
        if uses_dspark:
            ...
            if not target_enforce_eager or not draft_enforce_eager:
                raise RuntimeError(
                    "DeepSeek-V4 AFD DSpark currently supports only eager "
                    "target and eager draft execution"
                )
        else:
