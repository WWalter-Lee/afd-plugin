    ...
    *,
    ...
    if not hasattr(layer.mlp, "experts"):
        ...
        if not _FFN_COMPARE_WEIGHT_PRINTED and _ffn_compare_rank_enabled():
            ...
            w1 = shared_experts.gate_up_proj.weight
            ...
            print(
                ...
                f"w1_scale_shape={tuple(w1_scale.shape)}",
                f"w1_scale_dtype={w1_scale.dtype}",
                f"w1_scale_format={torch_npu.get_npu_format(w1_scale)}",
                f"w2_shape={tuple(w2.shape)}",
                f"w2_dtype={w2.dtype}",
                f"w2_format={torch_npu.get_npu_format(w2)}",
                f"w2_scale_shape={tuple(w2_scale.shape)}",
                f"w2_scale_dtype={w2_scale.dtype}",
                f"w2_scale_format={torch_npu.get_npu_format(w2_scale)}",
                flush=True,
            )
    ...
    if (
        ...
        print(
            ...
            f"w1_scale_shape={tuple(moe_weights.w1_scale[0].shape)}",
            f"w1_scale_dtype={moe_weights.w1_scale.dtype}",
            f"w1_scale_format={torch_npu.get_npu_format(moe_weights.w1_scale)}",
            ...
            f"w2_scale_shape={tuple(moe_weights.w2_scale[0].shape)}",
            f"w2_scale_dtype={moe_weights.w2_scale.dtype}",
            f"w2_scale_format={torch_npu.get_npu_format(moe_weights.w2_scale)}",
            flush=True,
        )
        _FFN_COMPARE_WEIGHT_PRINTED = True
