...
class AFDNPUFFNModelRunner(NPUModelRunner):
    ...
    def _compute_window_async_batch(
        ...
        if compare_enabled:
            self._ffn_compare_batch_printed = True
            ...
            for expert_id, token_count in states.group_list.cpu().tolist():
                ...
                layer_tokens[layer_idx] = (
                    layer_tokens.get(layer_idx, 0) + token_count
                )
                token_offset += token_count
                if token_offset >= actual:
                    break
            layer_ids = list(layer_tokens)
            print(
                "[FFN_COMPARE][BATCH]",
                f"rank={rank}",
                f"role_rank={role_rank}",
                f"layer_count={len(layer_ids)}",
                f"layers={layer_ids}",
                f"layer_tokens={[layer_tokens[layer] for layer in layer_ids]}",
